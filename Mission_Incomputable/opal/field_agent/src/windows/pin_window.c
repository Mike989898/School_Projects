...
static char* selection_handle_get_text(int index, void *context) {
  ...
  snprintf(
    ...
    sizeof(pin_window->field_buffs[0]), "%01x",
...
PinWindow* pin_window_create(PinWindowCallbacks callbacks) {
  ...
  if (pin_window) {
    ...
    if (pin_window->window) {
      ...
      text_layer_set_text(pin_window->main_text, "Enter 4-digit Hex CODE");
      ...
      text_layer_set_text(pin_window->sub_text, "Enter your CODE to continue");
      ...
      for (int i = 0; i < PIN_WINDOW_NUM_CELLS; i++) {
        selection_layer_set_cell_width(pin_window->selection, i, 30);
      }
